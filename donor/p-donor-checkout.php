<?php 
	...
	if(!empty($_GET['num'])) {
		...
		$stmt = $link->stmt_init();
		if ($stmt->prepare("SELECT Donation_Item.Value FROM Donation_Item WHERE Donation_Item.Person_ID=?")) {
			$stmt->bind_param("s", $pid);
			$stmt->execute();
					...
					transactions: [
				   {
					...
					// onAuthorize() is called when the buyer approves the payment
					 onAuthorize: function(data, actions) {
							...
							window.location = "https://hanahopedb.net//testview/donor/p-d-process.php?paymentID="+data.paymentID+"&payerID="+data.payerID+"&token="+data.paymentToken+"&pid=<?php echo $pid  ?>";
