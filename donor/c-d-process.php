...
// session and config removed for this application.
	...
	if(!empty($_GET['paymentID']) && !empty($_GET['payerID']) && !empty($_GET['token']) && !empty($_GET['pid']) ){
		...
		$sql = "UPDATE `Donation_Item` SET `Description`= '$des',`Thank_You_Status`= 'Sent' WHERE Donation_Item.Company_ID = $pid";
		if($link->query($sql)){
			$stmt = $link->stmt_init();
			if ($stmt->prepare("SELECT Company.Company_Name, Company.Contact_Name, Company.Email FROM Company WHERE Company.Company_ID=?")) {
			...
			$committeeMessage = "{$lname} at {$fname} has completed a cash donation.<br><br> {$type} {$value} {$des}";
		...
		// email the volunteer what role they signed up for and where they can find more information or make account changes.
			...
			$donorMessage = "Hello from the UCDenver Senior Design team! <br> You're receiving this email because you are on the spread sheet of contacts for the Hana Hope/ WhyXyte Wedding event charity. 
	...
	// close the link	
		...
		echo "problem";
		header('Location:https://hanahopedb.net//testview/donor/pdonation.html'); // Fail and go back to the donation page because the donation didn't go throughecho "all url values are missing";
	}
?>