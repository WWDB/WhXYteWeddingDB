...
// check all entered values for escape characters that may mess up database
	...
	//prevents blank data if someone goes back to this page. it'll redirect to the form 
	if ($Fname==""){
		header("Location: https://hanahopedb.net/testview/volunteer/volunteer.php");  // Redirect to login
		exit();
	}
...
// it's redirecting but not saying why. All attempts to notify user are failing
			...
			$sql2 = "INSERT INTO `Volunteer`(`Person_ID`, `Password`, `Volunteer_Status`) VALUES ('$person_id', '$password', '$Status')";
			if($link->query($sql2)){
